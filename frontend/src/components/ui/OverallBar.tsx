...
export function OverallBar({ value }: { value: unknown }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 mb-6 shadow-sm">
      <div className="flex justify-between items-end mb-4">
        <div>
          <h3 className="text-lg font-bold text-slate-900 dark:text-white leading-none">Overall Score</h3>
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">Weighted across all checks</p>
        </div>
        <div className={`text-4xl font-black ${colors.text} tracking-tighter leading-none drop-shadow-sm`}>
          {scoreText(value)}
        </div>
      </div>
      <div className={`h-4 w-full ${colors.lightBg} dark:bg-slate-800 rounded-full overflow-hidden shadow-inner`}>
        <div
